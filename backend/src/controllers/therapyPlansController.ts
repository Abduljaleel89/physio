...
export async function listTherapyPlans(req: AuthenticatedRequest, res: Response): Promise<void> {
  try {
    ...
    if (req.query.doctorId) {
      const did = (req.query.doctorId as string) === 'me' ? undefined : parseInt(req.query.doctorId as string);
      if (did) where.doctorId = did;
      if (!did && req.user.role === Role.PHYSIOTHERAPIST) {
...
export async function createTherapyPlan(req: AuthenticatedRequest, res: Response): Promise<void> {
  try {
    ...
    if (!doctorId && req.user.role === Role.PHYSIOTHERAPIST) {
      const doc = await prisma.doctor.findUnique({ where: { userId: req.user.id } });
      doctorId = doc?.id;
    }
...
export async function addExerciseToPlan(req: AuthenticatedRequest, res: Response): Promise<void> {
  try {
    ...
    let resolvedExerciseId: number | null = null;
    if (exerciseId) {
      ...
      const norm = (typeof difficulty === 'string' ? String(difficulty).toUpperCase() : undefined) as
        | 'BEGINNER'
        | 'INTERMEDIATE'
        | 'ADVANCED'
        | undefined;
    ...
    await prisma.therapyPlan.update({ where: { id: planId }, data: { version: newVersion } });
...
export async function reorderExercisesInPlan(req: AuthenticatedRequest, res: Response): Promise<void> {
  try {
    if (!req.user) {
      res.status(401).json({ success: false, error: "Authentication required" });
      return;
    ...
    const planId = parseInt(req.params.id);
    const items: Array<{ id: number; order: number }> = Array.isArray(req.body?.items) ? req.body.items : [];
    if (!items.length) {
      res.status(400).json({ success: false, error: "items array required" });
      return;
    }

    const plan = await prisma.therapyPlan.findUnique({ where: { id: planId }, include: { doctor: true } });
    if (!plan) {
      res.status(404).json({ success: false, error: "Therapy plan not found" });
      return;
    }
    if (req.user.role === Role.PHYSIOTHERAPIST && plan.doctor.userId !== req.user.id) {
      res.status(403).json({ success: false, error: "You can only edit your own plans" });
      return;
    }

    // Update in a transaction
    await prisma.$transaction(items.map((it) => prisma.therapyPlanExercise.update({ where: { id: it.id }, data: { order: it.order } })));

    // Bump version
    const newVersion = plan.version + 1;
    await prisma.therapyPlan.update({ where: { id: planId }, data: { version: newVersion } });
    await prisma.therapyPlanVersion.create({ data: { therapyPlanId: planId, version: newVersion, summary: 'Reordered exercises', createdBy: req.user.id } });
