...
async function getAdherenceAnalytics(req, res) {
    try {
        ...
        const where = { completedAt: { gte: start, lte: end }, undone: false };
        if (therapyPlanId) {
            const planExercises = await prisma_1.prisma.therapyPlanExercise.findMany({ where: { therapyPlanId }, select: { id: true } });
